...
I was setting up a project on GitHub and pushing up the code when
I came across this failure. 


```{}
rob@Taylor MI_Detection % git push -u origin main      
...
error: RPC failed; HTTP 400 curl 22 The requested URL returned error: 400
send-pack: unexpected disconnect while reading sideband packet
Writing objects: 100% (164/164), 8.52 MiB | 13.00 MiB/s, done.
Total 164 (delta 94), reused 0 (delta 0), pack-reused 0
fatal: the remote end hung up unexpectedly
Everything up-to-date

```

The fix was to increase a buffer size, see this link:

* \url{https://stackoverflow.com/questions/62753648/rpc-failed-http-400-curl-22-the-requested-url-returned-error-400-bad-request}

```{}
rob@Taylor MI_Detection % git config --global http.postBuffer 524288000
rob@Taylor MI_Detection % git config --global --list                   
user.name=Rob Donald
user.email=rob@statsresearch.co.uku
http.postbuffer=524288000
```

Then retry and all went well
```{}
rob@Taylor MI_Detection % git push -u origin main                      
Enumerating objects: 164, done.
Counting objects: 100% (164/164), done.
Delta compression using up to 10 threads
Compressing objects: 100% (163/163), done.
Writing objects: 100% (164/164), 8.52 MiB | 29.67 MiB/s, done.
Total 164 (delta 94), reused 0 (delta 0), pack-reused 0
remote: Resolving deltas: 100% (94/94), done.
To https://github.com/StatsResearch/MI_Detection.git
 * [new branch]      main -> main
branch 'main' set up to track 'origin/main'.


rob@Taylor MI_Detection % git push --tags
...
To https://github.com/StatsResearch/MI_Detection.git
 * [new tag]         DocRel_1.0 -> DocRel_1.0
 * [new tag]         Rel_1.0 -> Rel_1.0
